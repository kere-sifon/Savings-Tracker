/**
 * Legacy copy of the kids ledger seed data + logic.
 * The canonical runnable script is `scripts/seed-kids.ts` (`npm run seed:kids`).
 * This file is kept for reference or diff; prefer updating seed-kids.ts only.
 */
...
const MONGODB_URI =
  process.env.MONGODB_URI || "mongodb://localhost:27017/kids-account";
...
async function seed() {
  ...
  console.log(`💳 Inserted ${allTransactions.length} transactions (${income.length} income, ${expenses.length} expenses).`);
